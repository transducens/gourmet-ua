    ...
    def generate(
        ...
        **kwargs
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores = model.forward_decoder(tokens[:, :step + 1], encoder_outs,forced_factors)
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs,forced_factors):
        ...
        for model, encoder_out in zip(self.models, encoder_outs):
            probs, attn = self._decode_one(tokens, model, encoder_out, self.incremental_states,forced_factors, log_probs=True)
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states,forced_factors, log_probs):
